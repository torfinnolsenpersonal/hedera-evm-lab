...
// ============================================================================
// HAPI BENCHMARK - Hedera SDK token operations with 3 accounts
// ============================================================================
// Criteria:
// - 3 accounts
// - Create FT token
...
describe("HAPI Benchmark", function () {
  this.timeout(180000); // 3 minutes for HAPI operations
  ...
  const evidence: HAPIEvidence = {
    label: process.env.BENCHMARK_LABEL || "solo_hapi",
    network: "solo",
    operator_account: "",
    accounts: [],
    token_id: "",
  ...
  before(async function () {
    ...
    console.log("╠══════════════════════════════════════════════════════════╣");
    console.log("║  Test: FT token create + mint + transfer with 3 accounts ║");
  ...
  after(async function () {
    ...
    console.log(`[TIMING] hapi_total=${totalMs}`);
    for (const s of stepTimings) {
    ...
    for (const [step, txId] of Object.entries(evidence.transaction_ids)) {
      console.log(`[EVIDENCE] tx_${step}=${txId}`);
    }
    ...
    if (evidenceDir) {
      try {
        ...
        const artifactPath = path.join(evidenceDir, `${evidence.label}-hapi-evidence.json`);
  ...
  it("Step 1: Create 3 accounts", async function () {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (i === 0) {
        evidence.transaction_ids["create_account_0"] = tx.transactionId.toString();
      }
  ...
  it("Step 5: Transfer tokens between accounts", async function () {
    const start = Date.now();
  ...
  it("Step 6: Mirror node confirms balances", async function () {
    ...
    // Query balances via mirror node REST API
    ...
    console.log(`    account[0]: ${bal0 / 100} BENCH (expected: 80.00)`);
    console.log(`    account[1]: ${bal1 / 100} BENCH (expected: 10.00)`);
    console.log(`    account[2]: ${bal2 / 100} BENCH (expected: 10.00)`);

    recordStep("Mirror confirm", start);

    // Verify expected balances
    // account[0]: 10000 - 2000 = 8000 (80.00 tokens)
    // account[1]: 1000 (10.00 tokens)
    // account[2]: 1000 (10.00 tokens)
    expect(bal0).to.equal(8000, "account[0] balance mismatch");
    expect(bal1).to.equal(1000, "account[1] balance mismatch");
    expect(bal2).to.equal(1000, "account[2] balance mismatch");
  });

  it("Step 7: SDK query confirms balances", async function () {
    const start = Date.now();

    // Also verify via SDK AccountBalanceQuery
    const query0 = await new AccountBalanceQuery()
      .setAccountId(accounts[0].id)
      .execute(client);
    const query1 = await new AccountBalanceQuery()
      .setAccountId(accounts[1].id)
      .execute(client);
    const query2 = await new AccountBalanceQuery()
      .setAccountId(accounts[2].id)
      .execute(client);
