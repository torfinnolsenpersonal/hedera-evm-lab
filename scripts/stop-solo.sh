...
# Usage: ./stop-solo.sh [--keep-cluster]
# Flags:
#   --keep-cluster  Destroy Solo network but keep the kind cluster running
...
if $KEEP_CLUSTER; then
    echo ""
    echo -e "${YELLOW}--keep-cluster: Keeping kind cluster '${SOLO_CLUSTER_NAME}' running${NC}"
    echo ""
    echo -e "${GREEN}=== Solo network destroyed (cluster preserved) ===${NC}"
else
